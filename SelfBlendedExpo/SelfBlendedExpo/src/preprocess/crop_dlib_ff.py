...
def facecrop(org_path,save_path,face_detector,face_predictor,period=1,num_frames=10):
    ...
    for cnt_frame in range(frame_count_org): 
        ...
        save_path_=str(save_path/'frames'/os.path.basename(org_path).replace('.mp4','/'))
...
if __name__=='__main__':
    ...
    if args.dataset=='Original':
        dataset_path=Path.home()/'sharedscratch/sbi/data/FaceForensics++/original_sequences/youtube/{}/'.format(args.comp)
    elif args.dataset=='DeepFakeDetection_original':
        dataset_path=Path.home()/'sharedscratch/sbi/data/FaceForensics++/original_sequences/actors/{}/'.format(args.comp)
    elif args.dataset in ['DeepFakeDetection','FaceShifter','Face2Face','Deepfakes','FaceSwap','NeuralTextures']:
        dataset_path=Path.home()/'sharedscratch/sbi/data/FaceForensics++/manipulated_sequences/{}/{}/'.format(args.dataset,args.comp)
    elif args.dataset in ['Celeb-real','Celeb-synthesis','YouTube-real']:
        dataset_path=Path.home()/'sharedscratch/sbi/data/Celeb-DF-v2/{}/'.format(args.dataset)
    elif args.dataset in ['DFDC']:
        dataset_path=Path.home()/'sharedscratch/sbi/data/{}/'.format(args.dataset)
    else:
    ...
    movies_path_list=sorted(glob(str(movies_path/'*.mp4')))
