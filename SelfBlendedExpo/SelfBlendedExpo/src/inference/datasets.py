...
def init_ff(dataset='all',phase='test'):
	...
	original_path=Path.home()+'sharedscratch/sbi/data/FaceForensics++/original_sequences/youtube/raw/videos/'
	folder_list = sorted(glob(original_path+'*'))

	list_dict = json.load(open(Path.home()+'sharedscratch/sbi/data/FaceForensics++/{phase}.json','r'))
	filelist=[]
	for i in list_dict:
		filelist+=i
	...
	folder_list=[]
	for fake in fakes:
		fake_path=Path.home()+'sharedscratch/sbi/data/FaceForensics++/manipulated_sequences/{fake}/raw/videos/'
