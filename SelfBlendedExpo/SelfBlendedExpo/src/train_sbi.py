...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, _):
        try:
            ims=next(self.iter).get('img')
            lbs=next(self.iter).get('label')
        except StopIteration:
            self.iter = iter(self.gen)
            ims, lbs = next(self.iter)
            print("self.iter outputs: " + type(self.iter))
            print("ims outputs: " + type(ims))
            print("lbs outputs: " + type(lbs))
        try:

            ims = np.swapaxes(np.swapaxes(ims.numpy(), 1, 3), 1, 2)

            lbs = np.eye(self.ncl)[lbs].reshape(self.gen.batch_size*2, self.ncl)
            return ims, lbs
        except StopIteration:
            print("self.iter outputs: " + type(self.iter))
            print("ims outputs: " + type(ims))
...
def main(args):
    ...
    exit()
    tfTraining = DataGenerator(train_loader, 2)  # Assuming 2 classes
    tf_val = DataGenerator(val_loader, 2)
    
    print(type(tfTraining))
    model=Detector()
    now=datetime.now()
    ...
    model=model.to('cuda')
    #model_tf = tensorModel()  # Use your initialization function here
    #model_tf.compile(optimizer=tf.keras.optimizers.Adam(), loss='binary_crossentropy', metrics=['accuracy'])
    print("Num GPUs Available: ", len(tf.config.list_physical_devices('GPU')))
    #model_tf.fit(tfTraining, epochs=n_epoch, validation_data=tf_val)
    os.mkdir(save_path)
    #os.mkdir(save_path+"tensorflow/")
    #model_tf.save_weights(save_path+"tensorflow/")
    ...
    for epoch in range(n_epoch):
        ...
        print(f"number of test images:   {len(train_dataset)}" )
